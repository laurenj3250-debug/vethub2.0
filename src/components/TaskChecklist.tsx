    ...
    <div className="bg-slate-800/50 rounded-xl border border-slate-700/50 overflow-hidden">
      ...
      <div className="p-2 space-y-1.5">
        ...
        {Object.entries(pendingGroups).map(([taskName, group]) => {
          ...
          return (
            <div key={taskName} className="px-3 py-2 rounded-lg bg-slate-900/60 border border-slate-700/30">
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-sm font-medium text-white">{taskName}</span>
                {!group.isGeneral && totalInGroup > 0 && (
                  <span className="text-xs text-slate-400">{doneInGroup}/{totalInGroup}</span>
                )}
              </div>
              <div className="flex flex-wrap gap-1">
                {group.isGeneral && group.generalTask ? (
                  <button
                    ...
                    className="px-2 py-0.5 rounded text-xs bg-slate-600 text-white hover:bg-emerald-500 transition"
                  >
                    Mark Done
                  </button>
                ) : (
                  group.patients.map(({ patient, task }) => (
                    ...
                    >
                      {task.completed && '✓ '}{getPatientName(patient).split(' ')[0]}
                    </button>
                  ))
                )}
              </div>
